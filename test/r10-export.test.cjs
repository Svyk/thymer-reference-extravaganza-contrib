...
test('R10 plugin.js header declares v4.48.6', () => {
  assert.ok(source.startsWith('// v4.49.10'), 'first line must be // v4.49.10, got: ' + source.slice(0, 30));
});

test('R10 __REFX_VERSION runtime tell is 4.48.6', () => {
  assert.ok(source.includes('window.__REFX_VERSION = "4.49.10"'), '__REFX_VERSION must be 4.49.7');
});
