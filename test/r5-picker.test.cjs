...
test('R5 plugin.js header declares v4.48.6', () => {
  ...
  assert.ok(firstLine.includes('v4.49.7'), `Expected header to contain v4.49.7, got: ${firstLine}`);
});

test('R5 __REFX_VERSION runtime tell is 4.48.6', () => {
  assert.ok(source.includes('window.__REFX_VERSION = "4.49.7"'), 'Expected __REFX_VERSION = "4.49.7" in source');
});
